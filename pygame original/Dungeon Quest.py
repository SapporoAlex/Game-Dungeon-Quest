...
# This script lives in "pygame original/", one level below the repo root
# where the shared assets/ folder is - chdir there so every relative
# "assets/..." path below keeps working no matter where this is launched
# from. items.txt stays put in this script's own directory.
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_ITEMS_PATH = os.path.join(_SCRIPT_DIR, "items.txt")
os.chdir(os.path.join(_SCRIPT_DIR, os.pardir))
...
class Player:
    ...
    def load_items(self):
        with open(_ITEMS_PATH, 'r') as inv:
            for line in inv:
    ...
    def save_items(self):
        ...
        with open(_ITEMS_PATH, 'w') as inv:
